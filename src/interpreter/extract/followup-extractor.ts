...
function extractMark(text: string): 'line' | 'bar' | undefined {
  if (/折线图|折线展示|折线/.test(text)) return 'line';
  if (/柱状图|柱形图|柱图|柱状展示/.test(text)) return 'bar';
  return undefined;
}
...
export function extractFollowUpIntent(text: string): {
  ...
  const timeRangeMatch = text.match(/(?:改为|改成|缩短到|调整为|设为|改看|看)?\s*(?:最近|近|过去)?\s*(\d+|[一二三四五六七八九十]+)\s*(?:个交易日|个?天|日)/);
  if (timeRangeMatch) {
  ...
  const addMatch = text.match(/(?:再加上|同时加上|增加|再看看|再查看|加上|添加)\s*(.+?)(?:。|$)/);
  if (addMatch) {
